...
def process_pairs(pairs: list[PositionPair], anti_node_map: list[list[chr]]):
    ...
    for position_pair in pairs:
        ...
        anti_node_map[pos1.y][pos1.x] = "#"
        anti_node_map[pos2.y][pos2.x] = "#"

        diff_x = pos2.x - pos1.x
        diff_y = pos2.y - pos1.y

        anti_node_1_x = pos1.x
        anti_node_1_y = pos1.y

        while True:
            anti_node_1_x -= diff_x
            anti_node_1_y -= diff_y

            if 0 <= anti_node_1_x < size and 0 <= anti_node_1_y < size:
                print(f"[1] ({pos1},{pos2}) => {Position(pos1.x - diff_x, pos1.y - diff_y)}")
                anti_node_map[anti_node_1_y][anti_node_1_x] = "#"
                continue

            break

        anti_node_2_x = pos2.x
        anti_node_2_y = pos2.y

        while True:
            anti_node_2_x += diff_x
            anti_node_2_y += diff_y
